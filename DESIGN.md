...
- Pipeline: `analyze/url` (fetch+extract via oss2api) -> `generate/announcement` and
  `generate/blog-article` (LLM, from the extracted content) -> `post/bluesky`,
  `post/hatena-bookmark`, `post/aixsns` (thin wrappers over ksnsposter/aixec, no reimplemented
  posting logic). Hatena Blog and VWork Blog stay outside url2brain for now (git-push side effects).
